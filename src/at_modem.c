 ...
 *
 * Reprise d'OricTel (meme auteur) ; seule la base de temps change.
 */
...
/* --- Delai ~N ms ---------------------------------------------------------
 * Sur cible Neo6502 : boucle calibree en assembleur (neo_delay_ms, 6,25 MHz).
 * Sur hote (TEST_HOST) : no-op, le faux modem repond immediatement. */
#ifdef TEST_HOST
static void at_delay_ms(unsigned int ms) { (void)ms; }
#else
#include "neo_time.h"
#define at_delay_ms(ms) neo_delay_ms(ms)
#endif
